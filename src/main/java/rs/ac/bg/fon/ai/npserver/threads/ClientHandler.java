	...
	public void run() {
		...
		try {
			while (true) {
				...
				JsonObject request = (JsonObject) receiver.receive();
				JsonObject response = new JsonObject();
				switch (Operation.valueOf(request.get("operation").getAsString())) {
				case LOGIN:
					String username = request.get("username").getAsString();
					String password = request.get("password").getAsString();
					server.loggedIn(this);
					// response.setResult(KontrolerPL.getInstance().login(user.getUsername(),
					Korisnik user = KontrolerPL.getInstance().login(username, password);
					response.addProperty("username", user.getUsername());
					response.addProperty("password", user.getPassword());
					break;
